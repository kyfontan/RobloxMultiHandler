...
where gcc >nul 2>&1 || ( echo [ERROR] gcc not in PATH & exit /b 1 )
where g++ >nul 2>&1 || ( echo [ERROR] g++ not in PATH & exit /b 1 )
where windres >nul 2>&1 || ( echo [ERROR] windres not in PATH & exit /b 1 )
where dlltool >nul 2>&1 || ( echo [ERROR] dlltool not in PATH & exit /b 1 )
...
dlltool --kill-at -d libs\d3d11.def       -l libs\libd3d11.a       || exit /b 1
dlltool --kill-at -d libs\d3dcompiler.def -l libs\libd3dcompiler.a || exit /b 1
dlltool --kill-at -d libs\user32ext.def   -l libs\libuser32ext.a   || exit /b 1
...
windres resource.rc -O coff -o build/resource.res || exit /b 1
...
for %%F in (src\core\process.c src\core\uri.c src\core\json.c src\core\favorites.c src\core\settings.c src\core\anti_afk.c src\core\crash_detect.c src\core\singleton.c src\core\thumbnail.c) do (
    echo [c]   %%F
    gcc %CFLAGS% -c %%F -o build/%%~nF.o || exit /b 1
    set OBJS=!OBJS! build/%%~nF.o
)

echo [cpp] src\third_party\imgui\imgui.cpp
g++ %CXXFLAGS% %IMGUI_INC% -c src/third_party/imgui/imgui.cpp -o build/imgui.o || exit /b 1
set OBJS=!OBJS! build/imgui.o

echo [cpp] src\third_party\imgui\imgui_draw.cpp
g++ %CXXFLAGS% %IMGUI_INC% -c src/third_party/imgui/imgui_draw.cpp -o build/imgui_draw.o || exit /b 1
set OBJS=!OBJS! build/imgui_draw.o

echo [cpp] src\third_party\imgui\imgui_widgets.cpp
g++ %CXXFLAGS% %IMGUI_INC% -c src/third_party/imgui/imgui_widgets.cpp -o build/imgui_widgets.o || exit /b 1
set OBJS=!OBJS! build/imgui_widgets.o

echo [cpp] src\third_party\imgui\imgui_tables.cpp
g++ %CXXFLAGS% %IMGUI_INC% -c src/third_party/imgui/imgui_tables.cpp -o build/imgui_tables.o || exit /b 1
set OBJS=!OBJS! build/imgui_tables.o

echo [cpp] src\third_party\imgui\backends\imgui_impl_win32.cpp
g++ %CXXFLAGS% %IMGUI_INC% -c src/third_party/imgui/backends/imgui_impl_win32.cpp -o build/imgui_impl_win32.o || exit /b 1
set OBJS=!OBJS! build/imgui_impl_win32.o

echo [cpp] src\third_party\imgui\backends\imgui_impl_dx11.cpp
g++ %CXXFLAGS% %IMGUI_INC% -c src/third_party/imgui/backends/imgui_impl_dx11.cpp -o build/imgui_impl_dx11.o || exit /b 1
set OBJS=!OBJS! build/imgui_impl_dx11.o

echo [cpp] src\gui\Texture.cpp
g++ %CXXFLAGS% %IMGUI_INC% -c src/gui/Texture.cpp -o build/Texture.o || exit /b 1
set OBJS=!OBJS! build/Texture.o

echo [cpp] src\gui\Tabs.cpp
g++ %CXXFLAGS% %IMGUI_INC% -c src/gui/Tabs.cpp -o build/Tabs.o || exit /b 1
set OBJS=!OBJS! build/Tabs.o

echo [cpp] src\gui\App.cpp
g++ %CXXFLAGS% %IMGUI_INC% -c src/gui/App.cpp -o build/App.o || exit /b 1
set OBJS=!OBJS! build/App.o

echo [link] RobloxHandler.exe
g++ %LDFLAGS% !OBJS! %LIBS% -o RobloxHandler.exe || exit /b 1
